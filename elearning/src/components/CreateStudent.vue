<!-- <template>
          ...
          <v-main>
            <v-container>
              ...
              <v-form @submit.prevent style="width: 40%; margin: 0 auto; border-style: inherit;">
                <v-text-field v-model="studentIRN" label="Student IRN" style="height: 20%;"></v-text-field>
                <v-text-field v-model="studentName" label="Student name" style="height: 20%;"></v-text-field>
                ...
                <v-btn class="mt-2" type="submit" block  style="background-color: green; color: white;">Create</v-btn>
              </v-form>
            </v-container>
  ...
  const createStudent = async () => {
    try {
      await axios.post('https://localhost:7117/api/Student', {
